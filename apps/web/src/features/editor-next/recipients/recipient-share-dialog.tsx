...
export function RecipientShareDialog({
  ...
  const createDisabled = !draft.name.trim() || !draft.company.trim()
  ...
  const updateSelectedRecipient = (
    ...
    updateRecipient(selectedRecipient.id, input)
  }
  ...
  const copyEmbed = async () => {
    ...
    const html = createRecipientEmailEmbedHtml({ recipient: selectedRecipient, siteUri })
      ...
      {view === "new" ? (
        ...
        <div className="min-h-[96px] overflow-y-auto p-3">
          {recipients.length > 0 ? (
            ...
            <div className="flex flex-col items-center gap-2 rounded-lg border border-dashed border-border p-5 text-center">
...
function TabButton({
  ...
  return (
    <button
      className={cn(
        "flex h-full items-center border-b px-1 text-sm font-semibold transition-colors",
        active
...
function RecipientAvatar({
  ...
  const initial = recipient.name.trim().charAt(0).toUpperCase() || "R"
  ...
  return (
    ...
    >
      {logoUrl ? <AvatarImage alt="" className="rounded-md" src={logoUrl} /> : null}
      <AvatarFallback className="rounded-md bg-variable-background text-variable-foreground">
        {initial}
      </AvatarFallback>
    </Avatar>
  )
...
function RecipientEmbedPreview({
  ...
  const renderedLines = createRecipientPreviewLines(previewLines, recipient)

  return (
    <div className="aspect-[298/171] w-full overflow-hidden rounded-[10px] border border-border bg-background">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              <div className="truncate text-[10px] text-muted-foreground">
                Personalized Lightsite
              </div>
            </div>
          </div>
          <div className="rounded-lg border border-border bg-muted/25 p-2.5">
            <div className="mb-1 line-clamp-2 text-sm leading-tight font-semibold text-foreground">
              {renderedLines[0] ?? `${recipient.name}, this is for ${recipient.company}`}
            </div>
            <div className="flex flex-col gap-1 text-[10px] leading-snug text-muted-foreground">
              {(renderedLines.length > 1
                ? renderedLines.slice(1, 4)
                : [`Built for ${recipient.company}`, "Open the page to view the full one-pager."]
              ).map((line) => (
                <span key={line} className="line-clamp-1">
                  {line}
                </span>
              ))}
            </div>
...
function createRecipientPreviewLines(lines: string[], recipient: SiteRecipient) {
  return lines.map((line) =>
    line
      .replace(/\byou\b/gi, recipient.name)
      .replace(/\byour company\b/gi, recipient.company)
