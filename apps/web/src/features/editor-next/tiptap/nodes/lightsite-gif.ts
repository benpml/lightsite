...
export const LightsiteGif = Node.create({
  name: "gifBlock",
  group: "block",
  atom: true,
  selectable: true,
  ...
  addAttributes() {
    return {
      ...
      giphyId: {
        ...
        renderHTML: (attributes) => ({ "data-giphy-id": attributes.giphyId }),
      },
      height: { default: null },
      src: { default: "" },
      title: { default: "" },
  ...
  parseHTML() {
    return [{ tag: 'figure[data-lightsite-gif=""]' }]
  },

  renderHTML({ HTMLAttributes }) {
    return ["figure", mergeAttributes(HTMLAttributes, { "data-lightsite-gif": "" })]
  },
...
function createEmptyGifNodeView({
  ...
  dom.className = "lightsite-next-gif-empty"
  button.className = "lightsite-next-gif-empty-button"
  button.type = "button"
  title.className = "lightsite-next-gif-empty-title"
  title.textContent = "Select GIF"
  description.className = "lightsite-next-gif-empty-description"
