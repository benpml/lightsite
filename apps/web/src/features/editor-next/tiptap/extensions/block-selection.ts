...
import type { Selection, SelectionRange } from "@tiptap/pm/state"
...
export const LightsiteNextBlockSelection = Extension.create({
  ...
  addKeyboardShortcuts() {
    return {
      Backspace: ({ editor }) => deleteSelectedBlocks(editor.view),
      Delete: ({ editor }) => deleteSelectedBlocks(editor.view),
      "Mod-a": ({ editor }) => {
...
function deleteSelectedBlocks(view: EditorView) {
  ...
  const gridCellReplacementKeys = paragraphType
    ? getGridCellReplacementRangeKeys(doc, blockRanges)
    : new Set<string>()
